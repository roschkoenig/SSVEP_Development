...
for sb = 1:length(subs)
    ...
    if writeover || exist([Fmeeg fs sub '.mat'], 'file') == 0
        ...
        for t = 1:length(targettrg) try

            % Identify the correct behavioural file to load
            %------------------------------------------------------------------
            load(datafiles{trg == targettrg(t)}, 'EEG');    D = EEG;    clear EEG;
            ...
            if length(find(eventid == 4)) ~= length(B.TargOnT)
                warning('Not a consistent number of events');
            end
